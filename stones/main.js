...
class GameStrategyCalculator {
  ...
  calculateWinningPositions(totalStones, allowedMoves) {
    ...
    // Dynamic programming to find winning positions
  ...
  calculateOptimalMove(currentStones, allowedMoves) {
    // Find a move that puts opponent in losing position
    for (const move of allowedMoves) {
    ...
    // If no winning move, take the first available move
...
class StonesGameAI {
  ...
  determineFirstPlayer(totalStones, allowedMoves) {
    // 25% chance for random start, otherwise use optimal strategy
